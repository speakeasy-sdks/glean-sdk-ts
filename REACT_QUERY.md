...
```tsx
import { useClientImagesGet } from "@gleanwork/api-client/react-query/clientImagesGet.js";

export function Example() {
  const { data, error, status } = useClientImagesGet({});
...
import { invalidateClientImagesGet, invalidateAllClientImagesGet } from "@gleanwork/api-client/react-query/clientImagesGet.js";
      ...
      onSubmit={(e) => {
        ...
        mutate(formData, {
          onSuccess: () => {
            ...
            invalidateClientImagesGet(queryClient, /* ... arguments ... */);
            // OR, invalidate all cache entries for the query targets:
            invalidateAllClientImagesGet(queryClient);
          },
        });
      }}
    >
...
import { GleanCore } from "@gleanwork/api-client";
import { GleanProvider } from "@gleanwork/api-client/react-query";
import { useClientImagesGetSuspense } from "@gleanwork/api-client/react-query/clientImagesGet.js";
...
function Example() {
  const { data } = useClientImagesGetSuspense({});
...
import { GleanCore } from "@gleanwork/api-client";
import { prefetchClientImagesGet } from "@gleanwork/api-client/react-query/clientImagesGet.js";
...
export default async function Page() {
  ...
  await prefetchClientImagesGet(queryClient, glean, {});
