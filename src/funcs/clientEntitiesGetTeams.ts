 ...
 */
export function clientEntitiesGetTeams(
  client: GleanCore,
  request: operations.TeamsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.TeamsResponse,
    | GleanError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.TeamsRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.TeamsRequest, { explode: true });

  const path = pathToFunc("/rest/api/v1/teams")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.TeamsResponse$inboundSchema),
