 ...
 */
export function clientDocumentsRetrievePermissions(
  client: GleanCore,
...
> {
  const input: operations.GetdocpermissionsRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) => operations.GetdocpermissionsRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.GetDocPermissionsRequest, {
    explode: true,
  });
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.GetDocPermissionsResponse$inboundSchema),
