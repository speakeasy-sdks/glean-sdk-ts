 ...
 */
export function clientChatAsk(
  client: GleanCore,
  request: operations.AskRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.AskResponse,
    | errors.ErrorInfo
    | GleanError
    | SDKValidationError
    | UnexpectedClientError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.AskRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.AskRequest, { explode: true });

  const path = pathToFunc("/rest/api/v1/ask")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.AskResponse$inboundSchema),
