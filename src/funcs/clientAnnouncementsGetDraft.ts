 ...
 */
export function clientAnnouncementsGetDraft(
  client: GleanCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetdraftannouncementRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.GetAnnouncementRequest, {
    explode: true,
  });

  const path = pathToFunc("/rest/api/v1/getdraftannouncement")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.GetDraftAnnouncementResponse$inboundSchema),
