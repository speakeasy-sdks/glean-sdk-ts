 ...
 */
export function clientCollectionsEdit(
  client: GleanCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.EditdocumentcollectionsRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.EditDocumentCollectionsRequest, {
    explode: true,
  });

  const path = pathToFunc("/rest/api/v1/editdocumentcollections")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.EditDocumentCollectionsResponse$inboundSchema),
