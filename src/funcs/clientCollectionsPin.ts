 ...
 */
export function clientCollectionsPin(
  client: GleanCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.PincollectionRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.PinCollectionRequest, {
    explode: true,
  });

  const path = pathToFunc("/rest/api/v1/pincollection")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.GetCollectionResponse$inboundSchema),
