 ...
 */
export function clientDisplayableListsGet(
  client: GleanCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetdisplayablelistsRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.GetDisplayableListsRequest, {
    explode: true,
  });

  const path = pathToFunc("/rest/api/v1/getdisplayablelists")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.GetDisplayableListsResponse$inboundSchema),
