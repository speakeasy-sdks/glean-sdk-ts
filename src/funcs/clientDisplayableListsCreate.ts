 ...
 */
export function clientDisplayableListsCreate(
  client: GleanCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreatedisplayablelistsRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.CreateDisplayableListsRequest, {
    explode: true,
  });

  const path = pathToFunc("/rest/api/v1/createdisplayablelists")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.CreateDisplayableListsResponse$inboundSchema),
