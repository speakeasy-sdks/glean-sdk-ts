 ...
 */
export function clientAnswersGetBoard(
  client: GleanCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetanswerboardRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.GetAnswerBoardRequest, {
    explode: true,
  });

  const path = pathToFunc("/rest/api/v1/getanswerboard")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.GetAnswerBoardResponse$inboundSchema),
