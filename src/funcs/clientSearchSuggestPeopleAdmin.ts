 ...
 */
export function clientSearchSuggestPeopleAdmin(
  client: GleanCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.PeoplesuggestadminRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.PeopleSuggestRequest, {
    explode: true,
  });

  const path = pathToFunc("/rest/api/v1/peoplesuggestadmin")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.PeopleSuggestResponse$inboundSchema),
