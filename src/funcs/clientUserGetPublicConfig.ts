 ...
 */
export function clientUserGetPublicConfig(
  client: GleanCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.PublicconfigRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.PublicConfigRequest, {
    explode: true,
  });

  const path = pathToFunc("/rest/api/v1/publicclientconfig")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.ClientConfig$inboundSchema),
    M.fail([401, 429, "4XX"]),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
