 ...
 */
export function indexingTroubleshootingDatasourceDocumentDebug(
  client: GleanCore,
...
> {
  const input: operations.PostApiIndexV1DebugDatasourceDocumentRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.PostApiIndexV1DebugDatasourceDocumentRequest$outboundSchema
        .parse(value),
  ...
  const path = pathToFunc("/api/index/v1/debug/{datasource}/document")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.DebugDocumentResponse$inboundSchema, {
