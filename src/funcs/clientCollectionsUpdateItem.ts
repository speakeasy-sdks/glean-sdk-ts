 ...
 */
export function clientCollectionsUpdateItem(
  client: GleanCore,
...
> {
  const input: operations.EditcollectionitemRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) => operations.EditcollectionitemRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.EditCollectionItemRequest, {
    explode: true,
  });
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.EditCollectionItemResponse$inboundSchema),
