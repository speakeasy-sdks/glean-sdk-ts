 ...
 */
export function datasourcesUpdateDatasourceInstanceConfiguration(
  client: GleanCore,
...
> {
  const input: operations.UpdateDatasourceInstanceConfigurationRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.UpdateDatasourceInstanceConfigurationRequest$outboundSchema
        .parse(value),
  ...
  const body = encodeJSON(
    "body",
    payload.UpdateDatasourceConfigurationRequest,
    { explode: true },
  );

  const pathParams = {
    datasourceId: encodeSimple("datasourceId", payload.datasourceId, {
      explode: false,
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.DatasourceConfigurationResponse$inboundSchema),
