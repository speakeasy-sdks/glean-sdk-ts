 ...
 */
export function clientAnswersRetrieve(
  client: GleanCore,
...
> {
  const input: operations.GetanswerRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) => operations.GetanswerRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.GetAnswerRequest, { explode: true });
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.GetAnswerResponse$inboundSchema),
