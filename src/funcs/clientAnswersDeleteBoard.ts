 ...
 */
export function clientAnswersDeleteBoard(
  client: GleanCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.DeleteanswerboardsRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.DeleteAnswerBoardsRequest, {
    explode: true,
  });

  const path = pathToFunc("/rest/api/v1/deleteanswerboards")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.DeleteAnswerBoardsResponse$inboundSchema),
