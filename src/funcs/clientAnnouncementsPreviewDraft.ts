 ...
 */
export function clientAnnouncementsPreviewDraft(
  client: GleanCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.PreviewannouncementdraftRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.PreviewUgcRequest, { explode: true });

  const path = pathToFunc("/rest/api/v1/previewannouncementdraft")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.PreviewUgcResponse$inboundSchema),
