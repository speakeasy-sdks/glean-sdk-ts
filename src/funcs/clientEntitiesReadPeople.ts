 ...
 */
export function clientEntitiesReadPeople(
  client: GleanCore,
...
> {
  const input: operations.PeopleRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) => operations.PeopleRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.PeopleRequest, { explode: true });
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.PeopleResponse$inboundSchema),
