 ...
 */
export function clientGovernanceDataReportsStatus(
  client: GleanCore,
  id: string,
...
> {
  const input: operations.GetreportstatusRequest = {
    id: id,
  };

  const parsed = safeParse(
    input,
    (value) => operations.GetreportstatusRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/rest/api/v1/governance/data/reports/{id}/status")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.ReportStatusResponse$inboundSchema, {
