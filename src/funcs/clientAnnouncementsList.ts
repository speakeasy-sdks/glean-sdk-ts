...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListannouncementsRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.ListAnnouncementsRequest, {
    explode: true,
  });

  const path = pathToFunc("/rest/api/v1/listannouncements")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.ListAnnouncementsResponse$inboundSchema),
