 ...
 */
export function clientCollectionsMoveItem(
  client: GleanCore,
...
> {
  ...
  const body = encodeJSON("body", payload.MoveCollectionItemRequest, {
    explode: true,
  });

  const path = pathToFunc("/rest/api/v1/movecollectionitem")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.MoveCollectionItemResponse$inboundSchema),
    M.fail([400, 401, 422, 429, "4XX"]),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
