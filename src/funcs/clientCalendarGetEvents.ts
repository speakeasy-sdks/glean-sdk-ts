 ...
 */
export function clientCalendarGetEvents(
  client: GleanCore,
  request: operations.GeteventsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.GetEventsResponse,
    | GleanError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  ...
  const body = encodeJSON("body", payload.GetEventsRequest, { explode: true });

  const path = pathToFunc("/rest/api/v1/getevents")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.GetEventsResponse$inboundSchema),
