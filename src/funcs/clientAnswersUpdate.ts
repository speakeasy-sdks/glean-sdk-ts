 ...
 */
export function clientAnswersUpdate(
  client: GleanCore,
...
> {
  const input: operations.EditanswerRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) => operations.EditanswerRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.EditAnswerRequest, { explode: true });
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.Answer$inboundSchema),
