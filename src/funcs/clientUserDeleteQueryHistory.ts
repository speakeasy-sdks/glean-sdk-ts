 ...
 */
export function clientUserDeleteQueryHistory(
  client: GleanCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.DeletequeryhistoryRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.DeleteQueryHistoryRequest, {
    explode: true,
  });

  const path = pathToFunc("/rest/api/v1/deletequeryhistory")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.DeleteQueryHistoryResponse$inboundSchema),
