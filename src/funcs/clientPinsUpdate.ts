 ...
 */
export function clientPinsUpdate(
  client: GleanCore,
...
> {
  const input: operations.EditpinRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) => operations.EditpinRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.EditPinRequest, { explode: true });
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.PinDocument$inboundSchema),
