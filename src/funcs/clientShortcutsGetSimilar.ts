 ...
 */
export function clientShortcutsGetSimilar(
  client: GleanCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetsimilarshortcutsRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.GetSimilarShortcutsRequest, {
    explode: true,
  });

  const path = pathToFunc("/rest/api/v1/getsimilarshortcuts")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.GetSimilarShortcutsResponse$inboundSchema),
