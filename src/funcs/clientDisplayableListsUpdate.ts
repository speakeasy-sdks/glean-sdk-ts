 ...
 */
export function clientDisplayableListsUpdate(
  client: GleanCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdatedisplayablelistsRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.UpdateDisplayableListsRequest, {
    explode: true,
  });

  const path = pathToFunc("/rest/api/v1/updatedisplayablelists")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.UpdateDisplayableListsResponse$inboundSchema),
