 ...
 */
export function clientDocumentsGetAnalytics(
  client: GleanCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetdocumentanalyticsRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.GetDocumentAnalyticsRequest, {
    explode: true,
  });

  const path = pathToFunc("/rest/api/v1/getdocumentanalytics")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.GetDocumentAnalyticsResponse$inboundSchema),
