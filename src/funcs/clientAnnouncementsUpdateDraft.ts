 ...
 */
export function clientAnnouncementsUpdateDraft(
  client: GleanCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdatedraftannouncementRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.UpdateDraftAnnouncementRequest, {
    explode: true,
  });

  const path = pathToFunc("/rest/api/v1/updatedraftannouncement")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.Announcement$inboundSchema),
