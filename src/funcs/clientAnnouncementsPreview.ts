 ...
 */
export function clientAnnouncementsPreview(
  client: GleanCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.PreviewannouncementRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.StructuredTextMutableProperties, {
    explode: true,
  });

  const path = pathToFunc("/rest/api/v1/previewannouncement")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.PreviewStructuredTextResponse$inboundSchema),
