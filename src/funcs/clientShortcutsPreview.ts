 ...
 */
export function clientShortcutsPreview(
  client: GleanCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.PreviewshortcutRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.ShortcutMutableProperties, {
    explode: true,
  });

  const path = pathToFunc("/rest/api/v1/previewshortcut")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.PreviewShortcutResponse$inboundSchema),
