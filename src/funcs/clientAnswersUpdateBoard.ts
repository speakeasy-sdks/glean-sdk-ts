 ...
 */
export function clientAnswersUpdateBoard(
  client: GleanCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.EditanswerboardRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.EditAnswerBoardRequest, {
    explode: true,
  });

  const path = pathToFunc("/rest/api/v1/editanswerboard")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.EditAnswerBoardResponse$inboundSchema),
